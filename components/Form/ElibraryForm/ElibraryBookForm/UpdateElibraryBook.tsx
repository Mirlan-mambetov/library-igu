...
const UpdateElibraryBook: FC = () => {
	const theme = useTheme()
	const colors = tokens(theme.palette.mode)
	const { updateId, onClose } = useContext(MyModalContext)
	const { enqueueSnackbar } = useSnackbar()

	const [updateBook, { isLoading }] =
		elibraryApi.useUpdateElibraryBookMutation()
	...
	const submitHandler: SubmitHandler<ElibraryBookDto> = async (data) => {
		try {
			...
			await updateBook({ id: updateId, data: formData })
				.unwrap()
				.then(() => onClose())
		} catch (err) {
...
export default UpdateElibraryBook
