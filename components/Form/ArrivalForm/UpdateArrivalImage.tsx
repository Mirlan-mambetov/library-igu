import { MyModalContext } from '../../../contexts/MyModal.context'
import { isFetchBaseQueryError } from '../../../helpers/fetchBaseQueryError'
import { arrivalApi } from '../../../store/api/arrival/arrival.api'
import { tokens } from '../../../theme'
import { Field, isErrorWithMessage } from '../../UI'
import { IArrivalImageDto } from './updateArrivalImage.dto'
import { useTheme } from '@mui/material'
import Box from '@mui/material/Box'
import Button from '@mui/material/Button'
import Typography from '@mui/material/Typography'
import { useSnackbar } from 'notistack'
import { FC, useContext } from 'react'
import { SubmitHandler, useForm } from 'react-hook-form'

export const UpdateArrivalImage: FC = () => {
	const theme = useTheme()
	const { updateId, onClose } = useContext(MyModalContext)
	const colors = tokens(theme.palette.mode)
	const { enqueueSnackbar } = useSnackbar()
	const [updateArrival, { isLoading }] =
		arrivalApi.useUpdateArrivalImageMutation()

	const {
		register,
		handleSubmit,
		formState: { errors }
	} = useForm<IArrivalImageDto>({ mode: 'onChange' })

	const submitHanlder: SubmitHandler<IArrivalImageDto> = async (data) => {
		try {
			const file = data.image[0]
			if (!file.name.match(/\.(png|jpeg|jpg|gif)$/)) {
				console.log(file.type)
				return enqueueSnackbar(
					'Для файлов поддерживается форматы только png, jpeg, jpg, gif.',
					{ variant: 'error' }
				)
			}
			const formData = new FormData()
			formData.append('file', file)
			await updateArrival({ id: updateId, data: formData })
				.unwrap()
				.then(() => onClose())
		} catch (err) {
			if (isFetchBaseQueryError(err)) {
				const errMsg =
					// @ts-ignore
					'error' in err ? err.error : JSON.stringify(err.data.message)
				// @ts-ignore
				enqueueSnackbar(errMsg, { variant: 'error' })
			} else if (isErrorWithMessage(err)) {
				enqueueSnackbar(err.message, { variant: 'error' })
			}
		}
	}

	return (
		<form onSubmit={handleSubmit(submitHanlder)}>
			<Box sx={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
				<Typography variant='h5' color={colors.grey[100]}>
					Редактирование материала
				</Typography>
				<Field
					{...register('image', {
						required: 'Выберите файл'
					})}
					type='file'
					error={errors.image}
				/>
				<Button color='success' type='submit'>
					Отправить
				</Button>
			</Box>
		</form>
	)
}
